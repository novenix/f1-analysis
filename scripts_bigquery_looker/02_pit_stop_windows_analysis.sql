...
--            y clasificarlo en cohortes estratégicas (Early, Optimal, Late).
--
-- Para Looker: Vista base para análisis de estrategias, detallada por piloto.
-- ==============================================================================
...
race_results AS (
  SELECT
    ...
    r.Points,
    r.Status,
    -- Usar TIME_DIFF para convertir a segundos de forma segura
    TIME_DIFF(r.Time, TIME(0, 0, 0), SECOND) as race_time_seconds
...
SELECT
  ...
  -- Formateo de tiempo corregido
  CASE
    WHEN rr.race_time_seconds IS NOT NULL THEN
      CONCAT(
        CAST(TRUNC(rr.race_time_seconds / 3600) AS STRING), ':',
        LPAD(CAST(TRUNC(MOD(rr.race_time_seconds, 3600) / 60) AS STRING), 2, '0'), ':',
        FORMAT('%06.3f', CAST(MOD(rr.race_time_seconds, 60) AS FLOAT64))
      )
    ELSE NULL
...
LEFT JOIN circuit_stats cs
  ON ps.EventName = cs.EventName AND ps.Year = cs.Year
LEFT JOIN race_results rr
  ON ps.EventName = rr.EventName AND ps.Year = rr.Year AND ps.Driver = rr.Driver
...
--            (Early, Optimal, Late) a nivel de circuito.
--
...
WITH strategy_data AS (
  WITH pit_stops AS (
    SELECT
      ...
      MIN(LapNumber) OVER (PARTITION BY EventName, Year, Driver, CAST(Stint AS INT64)) as stint_start_lap
    FROM `topicos-bases-datos.f1_data_warehouse.laps_enriched`
    WHERE EventDate >= '2021-01-01' AND SessionName = 'Race' AND Deleted = FALSE AND Stint IS NOT NULL
  ),
  first_pit AS (
    SELECT
      EventName, Year, Driver, Team,
      MIN(CASE WHEN Stint = 2 THEN stint_start_lap END) as first_pit_lap
    FROM pit_stops GROUP BY EventName, Year, Driver, Team
  ),
  circuit_stats AS (
    SELECT
      EventName, Year,
    ...
    FROM first_pit WHERE first_pit_lap IS NOT NULL GROUP BY EventName, Year
  ),
  results AS (
    SELECT
  ...
  SELECT
    fp.EventName, fp.Year, fp.Driver, fp.Team, fp.first_pit_lap,
    CASE
      ...
      ELSE 'Late Stop'
    END as cohort,
    r.grid_position, r.final_position, r.grid_position - r.final_position as positions_gained, r.Points, r.Status
  FROM first_pit fp
...
SELECT
  ...
  ROUND(COUNTIF(Status = 'Finished') / COUNT(*) * 100, 1) as finish_rate_percent,
  ROUND(AVG(first_pit_lap), 1) as avg_pit_lap
FROM strategy_data
WHERE cohort != 'No Pit Stop'
...
-- ==============================================================================
-- Propósito: Analizar qué estrategia (parar antes o después que la media)
--            es más efectiva en cada circuito.
--
...
WITH race_data AS (
  WITH pit_stops AS (
    SELECT
      EventName, Year, Driver, Team, Stint, LapNumber,
      MIN(LapNumber) OVER (PARTITION BY EventName, Year, Driver, CAST(Stint AS INT64)) as stint_start
    FROM `topicos-bases-datos.f1_data_warehouse.laps_enriched`
    WHERE EventDate >= '2021-01-01' AND SessionName = 'Race' AND Deleted = FALSE AND Stint IS NOT NULL
  ),
  first_pit AS (
  ...
  SELECT
    fp.EventName, fp.Year, fp.first_pit_lap, r.grid, r.final_pos, r.grid - r.final_pos as gained,
    CASE
      WHEN fp.first_pit_lap < PERCENTILE_CONT(fp.first_pit_lap, 0.5) OVER (PARTITION BY fp.EventName, fp.Year)
      THEN 'Undercut (Early)'
      ELSE 'Overcut (Late)'
    END as strategy_type
  FROM first_pit fp
  JOIN results r ON fp.EventName = r.EventName AND fp.Year = r.Year AND fp.Driver = r.Driver
  WHERE fp.first_pit_lap IS NOT NULL
)
SELECT
  Year, EventName, strategy_type,
...
results_with_strategy AS (
  SELECT
    sc.EventName, sc.Year, sc.Driver, sc.Team, sc.total_stops,
    CASE
  ...
  JOIN `topicos-bases-datos.f1_data_warehouse.results` r
    ON sc.EventName = r.EventName AND sc.Year = r.Year AND sc.Driver = r.Abbreviation
  WHERE r.SessionName = 'Race' AND r.Position IS NOT NULL
)
SELECT
  Year, EventName, stop_strategy,
  COUNT(*) as drivers_count,
  ROUND(AVG(final_position), 2) as avg_final_position,
  ROUND(AVG(grid_position), 2) as avg_grid_position,
  ROUND(AVG(positions_gained), 2) as avg_positions_gained,
...
--            en cada circuito, agrupando por rangos de vueltas.
--
...
WITH historical_strategies AS (
  WITH pit_data AS (
    SELECT
      EventName, Year, Driver, Stint, LapNumber,
      MIN(LapNumber) OVER (PARTITION BY EventName, Year, Driver, CAST(Stint AS INT64)) as stint_start
    FROM `topicos-bases-datos.f1_data_warehouse.laps_enriched`
    WHERE EventDate >= '2021-01-01' AND SessionName = 'Race' AND Deleted = FALSE AND Stint IS NOT NULL
  ),
  first_pit AS (
    SELECT
      EventName, Year, Driver,
      MIN(CASE WHEN Stint = 2 THEN stint_start END) as pit_lap
    FROM pit_data GROUP BY EventName, Year, Driver
  ),
  results AS (
    SELECT
      r.EventName, r.Year, r.Abbreviation as Driver,
      CAST(r.Position AS INT64) as final_pos, r.Points
    FROM `topicos-bases-datos.f1_data_warehouse.results` r
    WHERE r.Year >= 2021 AND r.SessionName = 'Race' AND r.Position IS NOT NULL
  )
  SELECT
    fp.EventName, fp.Year, fp.pit_lap, r.final_pos, r.Points,
...
-- ==============================================================================
--
-- -- QUERY 2.1: Vista detallada de estrategias por piloto.
...
--
-- -- QUERY 2.2: Comparativa de cohortes (Early/Optimal/Late).
-- --   - Visualización: Barras agrupadas por cohorte.
-- --   - Eje X: cohort; Eje Y: avg_positions_gained o avg_points; Desglose: EventName.
--
-- -- QUERY 2.3: Undercut vs Overcut.
-- --   - Visualización: Barras comparativas.
-- --   - Métricas: success_rate_percent, avg_positions_gained. Comparar lado a lado por circuito.
--
-- -- QUERY 2.4: 1-Stop vs 2-Stop.
-- --   - Visualización: Barras apiladas o tabla.
-- --   - Muestra la distribución de estrategias por circuito.
--
-- -- QUERY 2.5: Guía de ventana óptima por circuito.
-- --   - Visualización: Heatmap o tabla de referencia.
-- --   - Usar como guía predictiva. Color por avg_points (verde = mejor ventana).
--
-- ==============================================================================